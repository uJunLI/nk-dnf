...
class BaseDictSet(Dataset):
    ...
    def __init__(self, data_path, image_list_file, patch_size=None, split='train', load_npy=True, repeat=1, raw_ext='ARW', max_samples=None, max_clip=1.0, min_clip=None, only_00=False, transpose=True, h_flip=True, v_flip=True, rotation=False, ratio=True, **kwargs):
        super().__init__()
            ...
            for (i, img_pair) in enumerate(f):
                ...
                if ((self.split == 'test') and self.only_00):
                    if (os.path.split(img_file)[(- 1)][5:8] != '_00'):
                        continue
                img_exposure = float(os.path.split(img_file)[(- 1)][9:(- 5)])
                lbl_exposure = float(os.path.split(lbl_file)[(- 1)][9:(- 5)])
                ratio = min((lbl_exposure / img_exposure), 300)
                self.img_info.append({'img': img_file, 'lbl': lbl_file, 'img_exposure': img_exposure, 'lbl_exposure': lbl_exposure, 'ratio': np.float32(ratio), 'iso': float(iso[3:]), 'focus': focus})
                if (max_samples and (i == (max_samples - 1))):
                    break
        print('processing: {} images for {}'.format(len(self.img_info), self.split))
        self.total_len = (len(self.img_info) * self.repeat)
        self.sampler = None
    def __len__(self):
    ...
    def __getitem__(self, index):
        ...
        if self.patch_size:
            ...
            if (self.split == 'train'):
                (yy, xx) = (jt.randint(0, ((H - patch_size) // self.block_size), (1,)), jt.randint(0, ((W - patch_size) // self.block_size), (1,)))
            else:
            ...
            if (self.h_flip and (jt.randint(0, 2, (1,)) == 1) and (self.split == 'train')):
                input_patch = np.flip(input_patch, axis=2)
                gt_patch = np.flip(gt_patch, axis=2)
                clean_raw_patch = np.flip(clean_raw_patch, axis=2)
            if (self.v_flip and (jt.randint(0, 2, (1,)) == 1) and (self.split == 'train')):
                input_patch = np.flip(input_patch, axis=1)
                gt_patch = np.flip(gt_patch, axis=1)
                clean_raw_patch = np.flip(clean_raw_patch, axis=1)
            if (self.transpose and (jt.randint(0, 2, (1,)) == 1) and (self.split == 'train')):
        ...
        return {'noisy_raw': noisy_raw, 'clean_raw': clean_raw, 'clean_rgb': clean_rgb, 'img_file': img_file, 'lbl_file': lbl_file, 'img_exposure': info['img_exposure'], 'lbl_exposure': info['lbl_exposure'], 'ratio': ratio}

@DATASET_REGISTRY.register()
class SonyDictSet(BaseDictSet):

    def __init__(self, data_path, image_list_file, patch_size=None, split='train', load_npy=True, raw_ext='ARW', repeat=1, max_samples=None, max_clip=1.0, min_clip=None, only_00=False, transpose=True, h_flip=True, v_flip=True, rotation=False, ratio=True, **kwargs):
        super(SonyDictSet, self).__init__(data_path, image_list_file, split=split, patch_size=patch_size, load_npy=load_npy, raw_ext=raw_ext, max_samples=max_samples, max_clip=max_clip, repeat=repeat, min_clip=min_clip, only_00=only_00, transpose=transpose, h_flip=h_flip, v_flip=v_flip, rotation=rotation, ratio=ratio)
        self.block_size = 2
...
class FujiDictSet(BaseDictSet):
    ...
    def __init__(self, data_path, image_list_file, patch_size=None, split='train', load_npy=True, raw_ext='RAF', repeat=1, max_samples=None, max_clip=1.0, min_clip=None, only_00=False, transpose=True, h_flip=True, v_flip=True, rotation=False, ratio=True, **kwargs):
        super(FujiDictSet, self).__init__(data_path, image_list_file, split=split, patch_size=patch_size, load_npy=load_npy, raw_ext=raw_ext, max_samples=max_samples, max_clip=max_clip, repeat=repeat, min_clip=min_clip, only_00=only_00, transpose=transpose, h_flip=h_flip, v_flip=v_flip, rotation=rotation, ratio=ratio)
        self.block_size = 3
