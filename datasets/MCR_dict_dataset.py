...
class MCRDictSet(Dataset):
    ...
    def __init__(self, data_path, image_list_file, patch_size=None, split='train', load_npy=True, repeat=1, raw_ext='ARW', max_samples=None, max_clip=1.0, min_clip=None, only_00=False, transpose=True, h_flip=True, v_flip=True, rotation=False, ratio=True, **kwargs):
        ...
        # add jittor
        self.total_len = (len(self.raw_input_path) * self.repeat)
        self.sampler = None
        ...
        if self.patch_size:
            ...
            if (self.split == 'train'):
                if (((H - patch_size) // self.block_size) > 0):
                    yy = jt.randint(0, ((H - patch_size) // self.block_size), (1,))
                else:
                    yy = 0
                if (((W - patch_size) // self.block_size) > 0):
                    xx = jt.randint(0, ((W - patch_size) // self.block_size), (1,))
                else:
                    xx = 0
            else:
    ...
    def _pack_raw(self, raw, hh=None, ww=None):
        ...
        if (self.patch_size is None):
            # out = np.concatenate((im[(:, 0:H:2, 0:W:2)], im[(:, 0:H:2, 1:W:2)], im[(:, 1:H:2, 1:W:2)], im[(:, 1:H:2, 0:W:2)]), axis=0)
            ...
            # out = np.concatenate((im[(:, h1:h2:2, w1:w2:2)], im[(:, h1:h2:2, (w1 + 1):w2:2)], im[(:, (h1 + 1):h2:2, (w1 + 1):w2:2)], im[(:, (h1 + 1):h2:2, w1:w2:2)]), axis=0)
